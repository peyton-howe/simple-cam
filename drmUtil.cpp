...
static int matchConfigToVisual(EGLDisplay display, EGLint visualId, EGLConfig *configs, int count)
{
	...
	eglGetConfigAttrib(drmegl.display, configs[i], EGL_RED_SIZE, &red_size);
        eglGetConfigAttrib(drmegl.display, configs[i], EGL_GREEN_SIZE, &green_size);
        eglGetConfigAttrib(drmegl.display, configs[i], EGL_BLUE_SIZE, &blue_size);
        eglGetConfigAttrib(drmegl.display, configs[i], EGL_ALPHA_SIZE, &alpha_size);	
...
static int init_EGL(void)
{
	...
	drmegl.display = eglGetDisplay((EGLNativeDisplayType)gbm_dev_);
	if (!drmegl.display)
		throw std::runtime_error("eglGetDisplay() failed");

	if (!eglInitialize(drmegl.display, &drmegl.major, &drmegl.minor))
	...
	if(!eglGetConfigs(drmegl.display, NULL, 0, &drmegl.num_configs) || drmegl.num_configs < 1)
		throw std::runtime_error("cannot get any configs\n");
		
	drmegl.config = (EGLConfig*)malloc(drmegl.num_configs * sizeof(EGLConfig));
		
	if (!eglChooseConfig(drmegl.display, conf_attribs, drmegl.config, drmegl.num_configs, &drmegl.num_configs))
		throw std::runtime_error("couldn't get an EGL visual config");
		
	int configIndex = matchConfigToVisual(drmegl.display, GBM_FORMAT_XRGB8888, drmegl.config, drmegl.num_configs);
	if (configIndex < 0){
		eglTerminate(drmegl.display);
		gbm_surface_destroy(gbm_surface_);
		gbm_device_destroy(gbm_dev_);
		throw std::runtime_error("Failed to find matching EGL config!\n");
	}
	
	drmegl.context = eglCreateContext(drmegl.display, drmegl.config[configIndex], EGL_NO_CONTEXT, ctx_attribs);
	if (drmegl.context == EGL_NO_CONTEXT)
	{
		eglTerminate(drmegl.display);
		gbmClean();
	...
	drmegl.surface = eglCreateWindowSurface(drmegl.display, drmegl.config[configIndex], gbm_surface_, NULL);
	if (drmegl.surface == EGL_NO_SURFACE)
	{
		eglDestroyContext(drmegl.display, drmegl.context);
		eglTerminate(drmegl.display);
		gbmClean();
	...
	eglMakeCurrent(drmegl.display, EGL_NO_SURFACE, EGL_NO_SURFACE, drmegl.context);
	int max_texture_size = 0;
	glGetIntegerv(GL_MAX_TEXTURE_SIZE, &max_texture_size);
	//max_image_width_ = max_image_height_ = max_texture_size;
	// This "undoes" the previous eglMakeCurrent.
	eglMakeCurrent(drmegl.display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);

	//printf("GL Extensions: \"%s\"\n", glGetString(GL_EXTENSIONS));
	eglMakeCurrent(drmegl.display, drmegl.surface, drmegl.surface, drmegl.context);
...
void makeBufferDRM(int fd, libcamera::StreamConfiguration const &info, libcamera::FrameBuffer *buffer, int camera_num)
{
	if (first_time_drm_)
	{
	// This stuff has to be delayed until we know we're in the thread doing the display.
		if (!eglMakeCurrent(drmegl.display, drmegl.surface, drmegl.surface, drmegl.context))
			throw std::runtime_error("eglMakeCurrent failed");
		gl_setup();
		glGenTextures(1, &drmegl.FramebufferName);
		glGenTextures(1, &drmegl.FramebufferName2);
		first_time_drm_ = false;
	}
	...
	EGLImage image = eglCreateImageKHR(drmegl.display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attribs);
	if (!image)
	...
	if (camera_num == 1){
		...
		//glDeleteTextures(1, &FramebufferName);
		//glGenTextures(1, &FramebufferName);
		glBindTexture(GL_TEXTURE_EXTERNAL_OES, drmegl.FramebufferName);
	}else if (camera_num == 2){
		//if (glIsTexture(FramebufferName2))
		//	glDeleteTextures(1, &FramebufferName2);
		//glDeleteTextures(1, &FramebufferName2);
		//glGenTextures(1, &FramebufferName2);
		glBindTexture(GL_TEXTURE_EXTERNAL_OES, drmegl.FramebufferName2);
	}
	...
	eglDestroyImageKHR(drmegl.display, image);
}
...
void displayDRM(int width, int height)
{
	...
	//eglSwapBuffers(egl_display_, egl_surface_);
	gbmSwapBuffers(&drmegl.display, &drmegl.surface);
}

void exit_drm(void)
{
	eglDestroyContext(drmegl.display, drmegl.context);
	eglDestroySurface(drmegl.display, drmegl.surface);
	eglTerminate(drmegl.display);
	gbmClean();
